...
export const exportToPython = project => {
  const zip = new JSZip();
  ...
  for (const i in keys) {
    ...
    zip.file(
      project.settings.folder + pascalCaseName + ".py",
  ...
  zip.generateAsync({ type: "blob" }).then(content => {
    saveAs(content, "unide-python-designs.zip");
  });
};

export const modelToPython = (pascalCaseName, tag, code) => {
  ...
  importedTags.forEach(tag => {
    importStrings = importStrings.concat(
      `from ${flowImports[tag].import} import ${flowImports[tag].name}\n`
    );
  });

  return `
  ${importStrings}
  class ${pascalCaseName}(Div)
    ${fields}
    ${internalClasses}
    def __init__(self):
        ${result}
